...
export function useGamepadMapping(gamepads: GamepadState[]) {
  ...
  const [editingDpad, setEditingDpad] = useState<{ gamepadIndex: number; direction: StickDirection } | null>(null)
  ...
  const setDpadMapping = useCallback((gamepadIndex: number, direction: StickDirection, key: string, label: string) => {
    setMappings(prev => {
      const updated = [...prev]
      let mapping = updated.find(m => m.gamepadIndex === gamepadIndex)
      ...
      const existingDpadMapping = mapping.dpadMappings.find(m => m.direction === direction)
  ...
  const removeDpadMapping = useCallback((gamepadIndex: number, direction: StickDirection) => {
    setMappings(prev => {
      const updated = [...prev]
      const mapping = updated.find(m => m.gamepadIndex === gamepadIndex)
      if (mapping && mapping.dpadMappings) {
        mapping.dpadMappings = mapping.dpadMappings.filter(m => m.direction !== direction)
      }
      return updated
    })
  }, [])
  ...
  useEffect(() => {
    gamepads.forEach(gamepad => {
      ...
      if (mapping.dpadMappings && mapping.dpadMappings.length > 0) {
        ...
        // Determine current dpad direction
        let currentDirection: StickDirection | null = null
        if (up && left) currentDirection = 'up-left'
        else if (up && right) currentDirection = 'up-right'
        else if (down && left) currentDirection = 'down-left'
        else if (down && right) currentDirection = 'down-right'
        ...
        mapping.dpadMappings.forEach(dpadMapping => {
          ...
          const isActive = currentDirection === dpadMapping.direction
